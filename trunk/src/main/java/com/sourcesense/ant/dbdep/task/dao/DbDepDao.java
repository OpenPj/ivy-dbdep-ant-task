...
public class DbDepDao implements IDbDepDao{
	...
	private boolean existDependency(final DependencyDO dependency){
		...
		try {
			...
			log.error("Error during checking dependency on this query: " +
					""+psSelect+ " | dependency: "+dependency,e);
			throw new RuntimeException("Error during checking dependency on this query: " +
					""+psSelect+ " | dependency: "+dependency,e);
		}
